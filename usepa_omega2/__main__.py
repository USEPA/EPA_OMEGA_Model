...
class ComplianceModel:
    ...
    def create_compliance_scenarios(self, mfr_name, mfr_graph, compliance_year, compliance_end_year):
        if compliance_year < compliance_end_year:
            ...
            for mfr_node_id in mfr_node_ids:
                if len(nx.descendants(mfr_graph, mfr_node_id)) == 0:
                    ...
                    self.create_compliance_scenarios(mfr_name, mfr_graph, compliance_year + 1, compliance_end_year)
        else:
            return
    ...
    def run_compliance(self):
        ...
        for mfr_name in self.manufacturer:
            ...
            self.create_compliance_scenarios(mfr_name, self.manufacturer[mfr_name], self.compliance_start_year, self.compliance_mid_year)
            print('')
...
if __name__ == "__main__":
    ...
    for mfr_name in cm.manufacturer:
        ...
        print('%d compliant nodes' % len(winning_node_ids))
        ...
        for i in pts:
            ...
            best_2024 = cm.manufacturer[mfr_name].nodes[winning_node_ids[i]]['mfr']
            ax1.plot(list(best_2024.emissions_target_net_co2_Mg.keys()),
                     list(best_2024.emissions_target_net_co2_Mg.values()) ,'b')
            print(best_2024)
            print(best_2024.production[target_year][0])
            print(best_2024.production[target_year][1])

            ax1.plot(list(best_2024.emissions_achieved_tailpipe_co2_Mg.keys()), list(best_2024.emissions_achieved_tailpipe_co2_Mg.values()))

            ax2.plot(list(best_2024.emissions_credits_running_tailpipe_co2_Mg.keys()), list(best_2024.emissions_credits_running_tailpipe_co2_Mg.values()))
