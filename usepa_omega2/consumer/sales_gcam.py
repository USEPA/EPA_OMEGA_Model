...
def demand_sales(session, model_year):
    ...
    for calendar_year in range(o2_options.analysis_initial_year, o2_options.analysis_final_year + 1):
        ...
        print(calendar_year)
        market_class_ids = session.query(DemandedSalesAnnualData.market_class_ID).filter(DemandedSalesAnnualData.calendar_year == calendar_year).distinct()
        for pass_num in range(1,3):
            for market_class_id in market_class_ids:
                ...
                vehicle_demanded_sales_annual_data = session.query(DemandedSalesAnnualData).filter(DemandedSalesAnnualData.calendar_year == calendar_year).filter(DemandedSalesAnnualData.market_class_ID == market_class_id[0])
                ...
                if (market_class_id[0]=='BEV non hauling'):
                    tmp_total_capital_costs = 45000 + 500 * (calendar_year - o2_options.analysis_initial_year)
                    tmp_annual_o_m_costs = 1600
                    tmp_fuel_cost_per_VMT = 0.03
                elif (market_class_id[0] == 'BEV hauling'):
                    tmp_total_capital_costs = 65000 + 500 * (calendar_year - o2_options.analysis_initial_year)
                    tmp_annual_o_m_costs = 1600
                    tmp_fuel_cost_per_VMT = 0.04
                elif (market_class_id[0] == 'ICE non hauling'):
                    tmp_total_capital_costs = 35000 + 1000 * (calendar_year - o2_options.analysis_initial_year)
                    tmp_annual_o_m_costs = 2000
                    tmp_fuel_cost_per_VMT = 0.10
                elif (market_class_id[0] == 'ICE hauling'):
                    tmp_total_capital_costs = 50000 + 1000 * (calendar_year - o2_options.analysis_initial_year)
                    tmp_annual_o_m_costs = 2000
                    tmp_fuel_cost_per_VMT = 0.10
