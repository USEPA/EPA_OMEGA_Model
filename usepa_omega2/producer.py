...
def calc_reg_class_demand(model_year):
    ...
    consumer_sales = consumer.demand_sales(model_year)
...
def calc_hauling_class_demand(model_year):
    ...
    return consumer.demand_sales(model_year)
...
def run_compliance_model():
    ...
    for manufacturer in o2.session.query(Manufacturer.manufacturer_ID).all():
        ...
        for calendar_year in range(o2.options.analysis_initial_year, o2.options.analysis_final_year + 1):
            ...
            market_shares_frac['hauling']['BEV'] = np.unique(np.linspace(0, 1, 5))
            market_shares_frac['non hauling']['BEV'] = np.unique(np.linspace(0, 1, 5))
            ...
            for hc in hauling_classes:
                ...
                for ms in ['BEV']:
                    ...
                    o2.session.execute(
                        'CREATE TABLE %s (%s_share_%s_frac FLOAT)' % (market_share_table_name, sql_valid_name(ms),
                                                                      sql_valid_name(hc)))
                    o2.session.execute('DELETE FROM %s' % market_share_table_name)  # clear table
            ...
            for hc in hauling_classes:
                ...
                # set BEV sales
                o2.session.execute('ALTER TABLE %s ADD COLUMN bev_%s_sales' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET bev_%s_sales=bev_share_%s_frac*%f' %
                                   (tech_share_combos_table_name, sql_valid_name(hc), sql_valid_name(hc),
                                    consumer.demand_sales(calendar_year)[hc]))

                # set ICE sales
                o2.session.execute('ALTER TABLE %s ADD COLUMN ice_%s_sales' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET ice_%s_sales=(1-bev_share_%s_frac)*%f' %
                                   (tech_share_combos_table_name, sql_valid_name(hc), sql_valid_name(hc),
                                    consumer.demand_sales(calendar_year)[hc]))
                ...
                cost_str = cost_str[:-1]
                cost_str = cost_str + ')'
                o2.session.execute('ALTER TABLE %s ADD COLUMN bev_%s_cost_dollars' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET bev_%s_cost_dollars=%s' %
                                   (tech_share_combos_table_name, sql_valid_name(hc), cost_str))

                if hc == 'hauling':
                    lifetime_vmt = o2.options.GHG_standard.calculate_cert_lifetime_vmt(reg_classes.truck, calendar_year)
                else:
                    lifetime_vmt = o2.options.GHG_standard.calculate_cert_lifetime_vmt(reg_classes.car, calendar_year)

                # tally up fake-o BEV Mg
                co2_Mg_str = '('
                for bev in bev_vehicles_by_market_class:
                    co2_Mg_str = co2_Mg_str + bev + '_co2_gpmi*bev_%s_sales*%f/1e6' % (
                        sql_valid_name(hc), lifetime_vmt) + '+'
                co2_Mg_str = co2_Mg_str[:-1]
                co2_Mg_str = co2_Mg_str + ')'
                o2.session.execute('ALTER TABLE %s ADD COLUMN bev_%s_co2_megagrams' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET bev_%s_co2_megagrams=%s' %
                ...
                cost_str = cost_str[:-1]
                cost_str = cost_str + ')'
                o2.session.execute('ALTER TABLE %s ADD COLUMN ice_%s_cost_dollars' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET ice_%s_cost_dollars=%s' %
                                   (tech_share_combos_table_name, sql_valid_name(hc), cost_str))

                # tally up fake-o ICE Mg
                co2_Mg_str = '('
                for ice in ice_vehicles_by_market_class:
                    co2_Mg_str = co2_Mg_str + ice + '_co2_gpmi*ice_%s_sales*%f/1e6' % (
                        sql_valid_name(hc), lifetime_vmt) + '+'
                co2_Mg_str = co2_Mg_str[:-1]
                co2_Mg_str = co2_Mg_str + ')'
                o2.session.execute('ALTER TABLE %s ADD COLUMN ice_%s_co2_megagrams' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET ice_%s_co2_megagrams=%s' %
                                   (tech_share_combos_table_name, sql_valid_name(hc), co2_Mg_str))

                # tally up total ICE/BEV combo cost
                o2.session.execute('ALTER TABLE %s ADD COLUMN %s_combo_cost_dollars' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET %s_combo_cost_dollars=bev_%s_cost_dollars+ice_%s_cost_dollars' %
                                   (tech_share_combos_table_name, sql_valid_name(hc),
                                    sql_valid_name(hc), sql_valid_name(hc))
                                   )

                # tally up total ICE/BEV Mg
                o2.session.execute('ALTER TABLE %s ADD COLUMN %s_combo_co2_megagrams' %
                                   (tech_share_combos_table_name, sql_valid_name(hc)))
                o2.session.execute('UPDATE %s SET %s_combo_co2_megagrams=bev_%s_co2_megagrams+ice_%s_co2_megagrams' %
                                   (tech_share_combos_table_name, sql_valid_name(hc),
                                    sql_valid_name(hc), sql_valid_name(hc))
                                   )
                ...
                megagrams_set = set()
                unique_data = []
                res = o2.session.execute('SELECT %s_combo_co2_megagrams, %s_combo_cost_dollars, * FROM %s' %
                                         (sql_valid_name(hc), sql_valid_name(hc),
            ...
            o2.session.execute(
                'ALTER TABLE tech_share_combos_total_%d ADD COLUMN total_combo_co2_megagrams' % calendar_year)
            o2.session.execute(
                'UPDATE tech_share_combos_total_%d SET \
                    total_combo_co2_megagrams=non_hauling_combo_co2_megagrams+hauling_combo_co2_megagrams' %
                calendar_year)
            ...
            if not o2.options.verbose:
                ...
                o2.session.execute(
                    'DELETE FROM tech_share_combos_total_%d WHERE total_combo_co2_megagrams NOT BETWEEN %f AND %f' %
