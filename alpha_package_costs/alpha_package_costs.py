...
def add_elements_for_package_key(input_df):
    ...
    #
    #
    # if df['Vehicle Type'][0] == 'Truck': df.insert(0, 'Structure Class', 'ladder')
    # else: df.insert(0, 'Structure Class', 'unibody')

    df.insert(df.columns.get_loc('Key'), 'Price Class', 1)
    return df


def calc_battery_kwh(settings, input_df):
    usable_soc = settings.bev_metrics_dict['usable_soc']['value']
    charging_loss = settings.bev_metrics_dict['charging_loss']['value']
    gap = settings.bev_metrics_dict['gap']['value']
    onroad_range = settings.onroad_bev_range_miles
    battery_kwh_list = list()
    for kwh_per_100_miles in input_df['Combined Consumption Rate']:
        battery_kwh_list.append(onroad_range / usable_soc * (kwh_per_100_miles / 100 * (1 - charging_loss)) / (1 - gap))
...
def calc_glider_weight(battery_weight_list, curb_weight_series):
...
def package_key(settings, input_df, fuel_id):
    ...
    if fuel_id == 'ice': pev_keys = pd.Series([0] * len(df))
        ...
        battery_kwh_list = calc_battery_kwh(settings, df)
        pev_keys = pd.Series(zip(pd.Series([settings.onroad_bev_range_miles] * len(df)),
                                 ...
                                 pd.Series([settings.bev_charging_loss] * len(df)),
    ...
    if fuel_id == 'ice':
        ...
        weight_keys = pd.Series(zip(curb_weights_series, curb_weights_series, battery_weight_list, df['Weight Reduction %']))
    if fuel_id == 'bev':
        battery_weight_list = calc_battery_weight(settings, battery_kwh_list)
        glider_weight_list = calc_glider_weight(battery_weight_list, curb_weights_series)
...
class PackageCost:
    ...
    def calc_bev_cost(self, pev_curves_dict):
        ...
        :return:
        """
        pev_range, pev_energy_rate, pev_soc, pev_charging_loss, pev_gap, battery_kwh, motor_power = self.pev_key
        battery_cost = battery_kwh * (pev_curves_dict['x_cubed_factor']['dollars_per_kWh_curve'] * battery_kwh ** 3 \
                                      + pev_curves_dict['x_squared_factor']['dollars_per_kWh_curve'] * battery_kwh ** 2 \
                                      + pev_curves_dict['x_factor']['dollars_per_kWh_curve'] * battery_kwh \
    ...
    def calc_weight_cost(self, weight_cost_dict, price_class_dict):
        ...
        weight_cost_cache_key = (curb_wt, weight_rdxn, self.price_key, self.structure_key)
...
def pev_package_results(settings, key, alpha_file_dict):
    ...
    fuel_key, alpha_key = pkg_obj.get_object_attributes(['fuel_key', 'alpha_key'])
    # oncycle_kwh_per_mile = battery_kwh * usable_soc * (1 - gap) / (onroad_range * (1 - charging_loss))
...
class SetInputs:
    ...
    gdp_deflators = pd.read_csv(path_input_templates / 'implicit_price_deflators.csv', skiprows=1, index_col=0)
...
def main():
    ...
    for idx, folder in enumerate(alpha_folders):
        ...
        for alpha_file in alpha_files[idx]:
            ...
            if folder.name.__contains__('BEV') and settings.run_bev:
                ...
                # bev_packages_df = pd.DataFrame()
                # for pkg_key in package_keys:
                #     package_result = pev_package_results(settings, pkg_key)
                #     bev_packages_df = pd.concat([bev_packages_df, package_result], axis=0, ignore_index=False)
                #
    ...
    if settings.run_ice:
        ...
        ice_packages_df = sum_vehicle_parts(ice_packages_df, settings.years,
                ...
                # fuel_key, alpha_key = pkg_obj.get_object_attributes(['fuel_key', 'alpha_key'])
                # if fuel_key != 'bev':
                #     ftp1, ftp2, ftp3, hwy, combined = alpha_file_dict[pkg_key]['EPA_FTP_1 gCO2/mi'], \
                #                                                           alpha_file_dict[pkg_key]['EPA_FTP_2 gCO2/mi'], \
                #                                                           alpha_file_dict[pkg_key]['EPA_FTP_3 gCO2/mi'], \
                #                                                           alpha_file_dict[pkg_key]['EPA_HWFET gCO2/mi'], \
                #                                                           alpha_file_dict[pkg_key]['Combined GHG gCO2/mi']
                # if fuel_key != 'ice':
                #     ftp1_kWh, ftp2_kWh, ftp3_kWh, hwy_kWh, combined_kWh = alpha_file_dict[pkg_key]['EPA_FTP_1_kWh/100mi'] / 100, \
                #                                                           alpha_file_dict[pkg_key]['EPA_FTP_2_kWh/100mi'] / 100, \
                #                                                           alpha_file_dict[pkg_key]['EPA_FTP_3_kWh/100mi'] / 100, \
                #                                                           alpha_file_dict[pkg_key]['EPA_HWFET_kWh/100mi'] / 100, \
                #                                                           alpha_file_dict[pkg_key]['Combined Consumption Rate'] / 100
                # if fuel_id != 'bev':
                #     engine_cost = pkg_obj.engine_cost(engine_cost_dict, startstop_cost_dict, boost_multiplier)
                #     trans_cost = pkg_obj.calc_trans_cost(trans_cost_dict)
                #     accessories_cost = pkg_obj.calc_accessory_cost(accessories_cost_dict)
                # ac_cost = pkg_obj.calc_ac_cost(ac_cost_dict)
                # if fuel_key == 'ice':
                #     # ice_cost = pkg_obj.calc_ice_cost(engine_cost_dict, startstop_cost_dict, boost_multiplier)
                #     ice_powertrain_cost = engine_cost + trans_cost + accessories_cost + ac_cost
                #     ice_powertrain_cost_df = pd.DataFrame(ice_powertrain_cost, columns=['ice_powertrain'], index=[pkg_key])
                #     pev_cost = 0
                #     pev_powertrain_cost_df = pd.DataFrame(pev_cost, columns=['pev_powertrain'], index=[pkg_key])
                #     powertrain_cost_df = ice_powertrain_cost_df.join(pev_powertrain_cost_df)
                # elif fuel_key == 'phev':
                #     # ice_cost = pkg_obj.calc_ice_cost(engine_cost_dict, startstop_cost_dict, boost_multiplier)
                #     ice_powertrain_cost = engine_cost + trans_cost + accessories_cost + ac_cost
                #     ice_powertrain_cost_df = pd.DataFrame(ice_powertrain_cost, columns=['ice_powertrain'], index=[pkg_key])
                #     pev_cost = pkg_obj.calc_phev_cost()
                #     pev_powertrain_cost_df = pd.DataFrame(pev_cost, columns=['pev_powertrain'], index=[pkg_key])
                #     powertrain_cost_df = ice_powertrain_cost_df.join(pev_powertrain_cost_df)
                # if fuel_key == 'bev':
                #     ice_powertrain_cost = 0
                #     ice_powertrain_cost_df = pd.DataFrame(ice_powertrain_cost, columns=['ice_powertrain'], index=[pkg_key])
                #     battery_cost, motor_cost, pev_cost = pkg_obj.calc_bev_cost(pev_curves_dict)
                #     pev_powertrain_cost = pev_cost + ac_cost
                #     pev_powertrain_cost_df = pd.DataFrame({'pev_battery': battery_cost, 'pev_motor': motor_cost, 'pev_powertrain': pev_powertrain_cost}, index=[pkg_key])
                #     powertrain_cost_df = ice_powertrain_cost_df.join(pev_powertrain_cost_df)
                # 
                # if fuel_id != 'bev':
                #     aero_cost = pkg_obj.calc_aero_cost(aero_cost_dict)
                #     nonaero_cost = pkg_obj.calc_nonaero_cost(nonaero_cost_dict)
                #     # roadload_cost = aero_cost + nonaero_cost
                #     # roadload_cost_df = pd.DataFrame(roadload_cost, columns=['roadload'], index=[pkg_key])
                # else:
                #     aero_cost, nonaero_cost = 0, 0
                # roadload_cost = aero_cost + nonaero_cost
                # roadload_cost_df = pd.DataFrame(roadload_cost, columns=['roadload'], index=[pkg_key])
                # 
                # weight_cost = pkg_obj.calc_weight_cost(weight_cost_dict, price_class_dict)
                # body_cost_df = pd.DataFrame(weight_cost, columns=['body'], index=[pkg_key])
                
    #             package_cost_df = powertrain_cost_df.join(roadload_cost_df).join(body_cost_df)
    #             package_cost_df.insert(0, 'cert_grams_per_mile', combined)
    #             package_cost_df.insert(0, 'hwfet_grams_per_mile', hwy)
    #             package_cost_df.insert(0, 'ftp_3_grams_per_mile', ftp3)
    #             package_cost_df.insert(0, 'ftp_2_grams_per_mile', ftp2)
    #             package_cost_df.insert(0, 'ftp_1_grams_per_mile', ftp1)
    #             package_cost_df.insert(0, 'cost_curve_class', f'{fuel_key}_{alpha_key}')
    #             all_packages_df = pd.concat([all_packages_df, package_cost_df], axis=0, ignore_index=False)
    #
    # all_packages_df = calc_year_over_year_costs(all_packages_df, 'ice_powertrain', years, learning_rate_ice_powertrain)
    # all_packages_df = calc_year_over_year_costs(all_packages_df, 'pev_powertrain', years, learning_rate_bev)
    # all_packages_df = calc_year_over_year_costs(all_packages_df, 'roadload', years, learning_rate_roadload)
    # all_packages_df = calc_year_over_year_costs(all_packages_df, 'body', years, learning_rate_weight)
    # all_packages_df.reset_index(drop=False, inplace=True)
    # all_packages_df.rename(columns={'index': 'package'}, inplace=True)
    #
    # all_packages_df = sum_vehicle_parts(all_packages_df, years,
    #                                     'new_vehicle_mfr_cost_dollars',
