...
class ManufacturerAnnualData(SQABase):
    ...
    calendar_year_cert_co2_Mg = Column('calendar_year_cert_co2_megagrams', Float)  #: certification CO2 (Mg) achieved in the given calendar year (initial compliance state)
    model_year_cert_co2_Mg = Column('model_year_cert_co2_megagrams', Float)  #: certificaiton CO2 (Mg) achieved, including credits transferred to/from other model years
    cert_target_co2_Mg = Column('cert_target_co2_megagrams', Float)  #: certification target CO2 (Mg) for the calendar year
    ...
    @staticmethod
    def create_manufacturer_annual_data(model_year, manufacturer_ID, cert_target_co2_Mg,
        ...
        Args:
            ...
            cert_target_co2_Mg (numeric): target CO2 Mg for the model year
            calendar_year_cert_co2_Mg (numeric): initial compliance state (CO2 Mg) of the vehicles produced in the model year
    ...
    def get_cert_target_co2_Mg():
        ...
        """
        return sql_unpack_result(omega_globals.session.query(ManufacturerAnnualData.cert_target_co2_Mg).all())
    ...
    def get_calendar_year_cert_co2_Mg():
        ...
        """
        return sql_unpack_result(omega_globals.session.query(ManufacturerAnnualData.calendar_year_cert_co2_Mg).all())
    ...
    def get_model_year_cert_co2_Mg():
        ...
        """
        return sql_unpack_result(omega_globals.session.query(ManufacturerAnnualData.model_year_cert_co2_Mg).all())
