    ...
    def calc_target_co2e_gpmi(vehicle):
        ...
        if cache_key not in VehicleTargets._data:
            ...
            if len(start_years[start_years <= vehicle.model_year]) > 0:
                ...
                # base curve targets
                base_coefficients = VehicleTargets._data['car', model_year]

                if vehicle.footprint_ft2 <= base_coefficients['fp_min']:
                    base_target_co2e_gpmi = base_coefficients['a_coeff']
                elif vehicle.footprint_ft2 > base_coefficients['fp_max']:
                    base_target_co2e_gpmi = base_coefficients['b_coeff']
                else:
                    base_target_co2e_gpmi = vehicle.footprint_ft2 * base_coefficients['c_coeff'] + base_coefficients['d_coeff']

                if (vehicle.reg_class_id == 'car') or (vehicle.cost_curve_class == 'BEV'):
                    target_co2e_gpmi = base_target_co2e_gpmi
                else:
                    ice_awd_offset_co2e_gpm = 10
                    bev_share_assumption = 0.5
                    if vehicle.footprint_ft2 <= 45:
                        ice_load_offset_cap_co2e_gpm = 0
                    elif vehicle.footprint_ft2 > 70:
                        ice_load_offset_cap_co2e_gpm = (9000-1500)/1000 * 8
                    else:
                        ice_load_offset_cap_co2e_gpm = (70 - vehicle.footprint_ft2)/(70 - 45) * (9000 - 1500) / 1000 * 8

                    if (vehicle.towing_capacity_lbs == 0) and (vehicle.context_size_class == 'Large Pickup'):
                        load_capacity_lbs = 8000
                    elif (vehicle.towing_capacity_lbs == 0) and (vehicle.context_size_class == 'Small Pickup'):
                        load_capacity_lbs = 3000
                    else:
                        load_capacity_lbs = vehicle.towing_capacity_lbs

                    ice_load_offset_co2e_gpm = max(0, min(ice_load_offset_cap_co2e_gpm, load_capacity_lbs))
                    target_co2e_gpmi = base_target_co2e_gpmi + bev_share_assumption * (ice_awd_offset_co2e_gpm + ice_load_offset_co2e_gpm)
